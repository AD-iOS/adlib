...
#ifndef _AD_FILENAME_HPP_
#define _AD_FILENAME_HPP_
...
std::string _AD_filename(const fs::path& path) {
    try {
        if (!fs::exists(path)) {
        ...
        std::cerr << "Error extracting filename: " << e.what() << std::endl;
        return "";
    }
}

std::string _AD_filename_base(const fs::path& path) {
    try {
        if (!fs::exists(path)) {
            std::cerr << "Path does not exist: " << path << std::endl;
            return "";
        ...
        size_t first_dot_pos = filename.find_first_of('.');
        
        if (first_dot_pos == std::string::npos || first_dot_pos == 0) {
            return filename;
        }
        ...
        std::cerr << "Error extracting filename base: " << e.what() << std::endl;
        return "";
    }
}
