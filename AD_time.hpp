...
#ifndef _AD_TIME_HPP_
#define _AD_TIME_HPP_
...
public:
    stopwatch(const std::string& name = "Task", bool print_on_destruct = true) 
        : task_name(name), auto_print(print_on_destruct) {
    ...
    ~stopwatch() {
        if (auto_print) {
            ...
            AD::cout << "[" << task_name << "] Elapsed: " << ms << " ms" << AD::endl;
        }
    }
...
inline stopwatch StopWatch("GlobalStopwatch", false);
